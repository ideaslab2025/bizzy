
// Demo system exports
export { default as DemoContainer } from './DemoContainer';
export { demoAnimations, gestureConfig, reducedMotionVariants } from './DemoAnimations';
export { useDemoContent } from '../../hooks/useDemoContent';
export { useLiveGuidanceContent } from '../../hooks/useLiveGuidanceContent';
export { useLiveDocuments } from '../../hooks/useLiveDocuments';
export { useLiveAIContent } from '../../hooks/useLiveAIContent';
export { useLiveDashboard } from '../../hooks/useLiveDashboard';
export type { DemoContent, DemoTab, DemoState, DemoInteraction } from '../../types/demo';
